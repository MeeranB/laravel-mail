...
class BookingPropertyCancelled extends BaseTemplate {
    ...
    private function emailTable() {
        ...
        return [
            ...
            'right'=> [
                ...
                'Total:' => $isConsortia ? sprintf('%s %s', $currencySymbol, number_format($this->booking->getCommissionlessTotal(), 2, '.', '')) : sprintf('%s %s', $currencySymbol, number_format($this->booking->total, 2 ,'.', '')),
                'Discount**:' =>!$isConsortia && $this->booking->getDiscountAmount() ? sprintf('%s %s', $currencySymbol, number_format($this->booking->getDiscountAmount(), 2, '.', '')) : false,
                'Deposit:' => !$isConsortia ? sprintf('%s %s', $currencySymbol, number_format($this->booking->getTotalPaid(), 2, '.', '')) : false,
                'Cancellation Fees:' => !$isConsortia ? sprintf('%s %s', $transactionalCurrencySymbol, number_format(number_format($this->booking->getCancellationAmount($this->booking->stamp_cancelled), 2, '.', '') * number_format($this->booking->exchange_rate, 2, '.', ''), 2, '.', '')) : false,
                'Refund:' => !$isConsortia && $this->booking->getRefundedTotal(false) ? sprintf('%s %s', $currencySymbol, number_format(abs($this->booking->getRefundedTotal(false), 2, '.', ''))) : false
            ],
            'outline' => true
        ];
    }
    ...
    public function build() {
        ...
        return $this->markdown('mail.templates.' . $this->template)
                    ...
                    ->subject(sprintf('Booking cancelled %s', $this->booking->uid));
    }
}